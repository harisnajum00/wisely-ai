...
export default function EmptyState({ onSuggestionClick }: EmptyStateProps) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 py-6 sm:py-12 min-h-0">
      ...
      {/* Suggestion Cards — 2 cols on mobile, 3 on tablet, 5 on desktop */}
      <motion.div
      ...
      >
        {suggestions.map((suggestion, index) => (
          <motion.button
            ...
            className="group glass rounded-xl sm:rounded-2xl p-3 sm:p-4 text-left transition-all duration-300 hover:glow-border cursor-pointer"
          >
            <div
              className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-gradient-to-br ${suggestion.gradient} flex items-center justify-center mb-2 sm:mb-3 group-hover:scale-110 transition-transform duration-300`}
            >
              <suggestion.icon className="size-4 sm:size-5 text-white" />
            </div>
            <h3 className="text-xs sm:text-sm font-medium text-foreground/80 group-hover:text-foreground transition-colors leading-tight">
              {suggestion.title}
            </h3>
            <p className="text-[10px] sm:text-xs text-muted-foreground/50 group-hover:text-muted-foreground/70 transition-colors line-clamp-2 hidden sm:block mt-0.5">
              {suggestion.prompt}
            </p>
          </motion.button>
        ))}
